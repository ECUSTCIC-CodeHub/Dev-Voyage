...
#### 1.5.3 配置 Clash Verge Rev（GUI 客户端，以 Windows/macOS 为例）

对于不熟悉命令行的用户，使用图形界面客户端是更便捷的选择。**Clash Verge Rev** 是目前持续维护的热门 GUI 客户端，支持 Windows、macOS 和 Linux。

##### 下载与安装

1.  **下载地址**：
    - **GitHub Releases（官方）**：访问 [clash-verge-rev 官方 Releases 页](https://github.com/clash-verge-rev/clash-verge-rev/releases) 下载最新版本。
    - **镜像加速**：如果 GitHub 下载慢，可使用前述文件加速镜像（如 `ghp.ci`）粘贴下载链接。
    - **用户中心**：部分机场服务商也会提供封装好的客户端安装包。

2.  **选择对应版本**：
    - **Windows**：一般选择 `Clash.Verge_{版本号}_x64-setup.exe`（64位系统）。
    - **macOS**：
        - Intel 芯片：选择 `x64.dmg` 文件。
        - Apple Silicon 芯片（M1/M2/M3）：选择 `aarch64.dmg` 文件。
    - **Linux**：根据发行版选择 `.deb`（Debian/Ubuntu）或 `.rpm`（Fedora/RHEL）文件。

3.  **安装注意事项**：
    - **Windows**：建议临时退出杀毒软件（如 360、腾讯管家），避免误拦截。
    - **macOS**：如果提示“无法打开”，需要在“系统设置”→“隐私与安全性”中点击“仍要打开”，或在终端执行 `sudo xattr -d com.apple.quarantine /Applications/Clash\ Verge.app`。

##### 导入订阅

1.  打开 Clash Verge Rev，点击左侧 **订阅** 标签页。
2.  在 **订阅文件链接** 输入框中，粘贴从机场服务商获取的 Clash 订阅链接。
3.  点击 **导入** 按钮，软件会自动下载配置文件。
4.  导入成功后，可在下拉菜单中看到多个节点入口。

##### 选择代理模式

点击左侧 **代理** 标签页，在右上角选择模式：

| 模式             | 说明                        | 适用场景                       |
| -------------- | ------------------------- | -------------------------- |
| **规则（Rule）**   | 根据预设规则自动判断：国内网站直连，国外网站代理。 | **日常首选**，兼顾速度与稳定性。         |
| **全局（Global）** | 所有流量都通过代理。                | 访问某些需要特定地区 IP 的服务（如部分流媒体）。 |
| **直连（Direct）** | 所有流量都不通过代理。               | 临时关闭代理，或排查网络故障。            |

##### 启动代理

1.  点击左侧 **设置** 标签页。
2.  在 **系统设置** 区域，开启 **系统代理** 开关。
3.  （可选）如果某些应用（如 UWP 应用、命令行工具）不走系统代理，可以开启 **虚拟网卡模式（TUN 模式）**。
4.  开启后，任务栏或菜单栏会出现 Clash 图标（小猫），右键可快速切换节点或模式。

##### 验证代理是否生效
打开浏览器访问 [https://ip.sb](https://ip.sb) 或 [https://www.whatismyip.com](https://www.whatismyip.com)，显示的 IP 地址应为代理服务器的 IP，而非本地网络 IP。
...
以下是一些长期维护、信誉较好的免费订阅聚合站，可定期获取临时节点：

| 名称 | 网址 | 特点 |
|------|------|------|
| **ClashNode** | https://clashnode.com | 每日更新免费节点，支持多种协议。 |
| **Free Node** | https://freenode.cc | 提供 Clash 订阅链接和节点列表。 |
| **NodeFree** | https://nodefree.org | 更新频率高，节点较多。 |
| **GitHub 开源项目** | 搜索 `free-proxy`、`clash-node` 等关键词 | 许多开发者会分享自用节点。 |
...
1.  从上述渠道获取 `订阅链接` 或单个 `节点信息`。
2.  在 Clash Verge Rev 的订阅页面粘贴链接导入（同 1.5.3 步骤）。
3.  如果获取的是单个节点，可在订阅页面点击 **粘贴解析** 或手动添加。
