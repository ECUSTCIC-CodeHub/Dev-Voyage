...
Windows 从 1985 年到现在，一直保持**向后兼容**。这意味着：
- 30 年前的软件理论上还能运行
- 但也带来了巨大的安全隐患
- 任何软件都能修改系统关键位置
...
❌ 奇怪位置：D:\乱七八糟的文件夹\...
```
- Windows 软件默认装 C 盘 Program Files
- macOS 软件拖到"应用程序"文件夹
- Linux 软件默认由包管理器决定
...
**iOS：**
1. 长按 APP 图标 → 删除应用
2. 或设置 → 通用 → iPhone 储存空间 → 删除应用

---

::: tip 历史趣事：软件的"共享软件"时代
...
A：
1. Windows：设置 → 应用 → 应用和功能（按安装日期排序）
2. 问自己：这个软件我上次用是什么时候？
3. 如果答案是"3 个月前"，大概率可以卸载
...
1. **检查已安装软件**：
   ...
   - 找出最近 3 个月安装的，问自己"我真的需要这个吗？"
2. **卸载一个垃圾软件**：
   - 找找有没有"XX 助手"、"YY 加速"、"ZZ 日历"
   - 按正确方法卸载它
3. **练习安全下载**：
   - 访问 `python.org`（Python 官网）
   - 找到正确的下载按钮（不是广告！）
   - 下载但不安装（除非你真需要）

### 🚀 进阶挑战
1. **使用包管理器安装软件**：
   - Windows：尝试 Winget `winget install Microsoft.PowerToys`
   - macOS：安装 Homebrew 并用它装一个软件
2. **创建系统还原点**（Windows）：
   - 搜索"创建还原点"
   - 在安装未知软件前创建一个
   - 学会如何还原
...
现在你的软件管理得服服帖帖了，但**Wi-Fi 连不上？先试试"重启大法"**？

下一章：[Wi-Fi 连不上？先试试"重启大法"](./4-wifi-troubleshooting.md)
