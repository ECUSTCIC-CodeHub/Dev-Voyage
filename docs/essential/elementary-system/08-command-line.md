---
title: '第 8 章：命令行的神秘世界：黑客不是敲键盘最快的人'
...
本章难度较大，且日常使用不会有太多相关问题，可选择跳过。

如果你想学习命令行，可以看看我们的[命令行教程](/essential/command-line/)
:::
...
**历史原因：**
早期的计算机只有命令行（1960-1980 年代），图形界面是后来才有的。所以：
- 老一辈程序员习惯了命令行
- 很多专业工具只有命令行版本
- 服务器环境通常没有图形界面
...
2. **程序之间可以通过管道连接**
   ...
   - 示例：`ls | grep "txt" | sort`（列出文件，过滤出 txt 文件，排序）
...
```powershell
ls  # 或 dir，两者都可以
Get-ChildItem  # 完整的 PowerShell 命令
```
...
```bash
# 先看例子
命令 --help | head -20  # 只看前 20 行，通常例子在前面
```
...
2. **理解命令含义**：不要运行看不懂的命令
3. **备份重要数据**：操作前备份
4. **使用 --dry-run 参数**：很多命令支持"模拟运行"，不实际执行
...
A：大多数命令是**小写**的，因为 Unix/Linux 系统区分大小写。习惯就好：
- `LS` 和 `ls` 可能是完全不同的东西
- 新手常犯的错误：把命令打成了大写
...
现在你已经是命令行入门选手了，但**电脑卡了？先别砸！**？

下一章：[电脑卡了？先别砸！](./09-performance-optimization.md)
