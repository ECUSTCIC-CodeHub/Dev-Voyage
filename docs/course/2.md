...
Node.js 是构建现代前端应用的基础，使用 nvm (Node Version Manager) 可以方便地管理多个 Node.js 版本：
...
Python 是一门强大的后端开发语言，pyenv 可以帮助我们管理多个 Python 版本：
...
# 安装 Python 3.11（本课程推荐版本）
pyenv install 3.11.0
pyenv global 3.11.0

# 验证安装是否成功
python --version  # 应该显示 Python 3.11.0
pip --version     # 应该显示类似 pip 21.x.x 的版本号
```

### 包管理工具配置

合理使用包管理工具可以提高开发效率并避免依赖冲突：

```bash
# 前端包管理 - 推荐使用 pnpm（性能优于 npm 和 yarn）
npm install -g pnpm

pip install pdm  # 后端包管理 - 推荐使用 pdm（性能优于 pip）
```
...
合理配置环境变量对于保护敏感信息和适应不同环境至关重要：

```bash
# 前端环境变量 (.env.local)
NUXT_API_BASE_URL=http://localhost:8000  # API 基础地址
NUXT_PUBLIC_SITE_NAME=My Personal Website  # 站点名称

# 后端环境变量 (.env)
DATABASE_URL=sqlite:///./app.db  # 数据库连接字符串
SECRET_KEY=your-secret-key-here  # 用于加密的密钥（注意保密）
```
...
FastAPI 是一个现代化、高性能的 Python Web 框架，适用于构建 API。以下是初始化 FastAPI 项目的详细步骤：
